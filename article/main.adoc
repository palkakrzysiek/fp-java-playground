...
It turn out the code compiles with each of our mappers
...
Because we can use a function that can produce a `Vehicle` or something more concrete like `Car` from `Steel` and we can't complain if `RawMaterial` is enough for it.
In other words a function B is a subtype if a function A if the function B returns a subtype of the function A and takes a supertype of the function A.
