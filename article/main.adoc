...
Now by calling getAvgAge we aren't even informed that something can go wrong, so it's easy to forget to handle an error. The argument of easier reasoning about a program written in the functional style apparently doesn't work here.
