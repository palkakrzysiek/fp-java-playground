...
If we're into asynchronous processing, we can switch `Try` to `Future` (the vavr's one, not the Java one which has method naming inconsistent with `Stream`/`Option` part). However, then we must be careful not to call `.get` or we'd block the execution. What can be done is to use `.onSuccess` and `.onFailure` to complete the `Promise` of whatever framework or library expects.
