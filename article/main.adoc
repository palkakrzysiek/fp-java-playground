...
To ease the pain of "mutating" immutable objects one can add a builder with `toBuilder` argument set to `true`. Unfortunately, it's not a default value. Mentioned annotation allows summoning the builder with pre-filled fields from an existing object.
