...
The first thing here is to use `Try.traverse` instead of mapping each name in the list like `name -> cs::getByNameWithTry`
...
how the patter matching could look like http://blog.vavr.io/integrating-partial-functions-into-javaslang
